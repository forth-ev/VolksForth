...
\atari T{ MIN-INT MIN-INT M* MIN-INT FM/MOD -> 0 MIN-INT }T
...
\atari T{ MIN-INT MIN-INT M* MIN-INT SM/REM -> 0 MIN-INT }T
...
ALIGN
\atari T{ HERE 1 ALLOT ALIGN HERE SWAP - ALMNT = -> <TRUE> }T
\ End of extra test
...
\atari T{ UA-ADDR ALIGNED -> A-ADDR }T
...
T{ BL -> 20 }T
T{ CHAR X -> 58 }T  \ vf: for CBM: s/X/x/
T{ CHAR HELLO -> 48 }T  \ vf: for CBM: s/HELLO/hello/
T{ : GC1 [CHAR] X ; -> }T  \ vf: for CBM: s/X/x/
T{ : GC2 [CHAR] HELLO ; -> }T  \ vf: for CBM: s/HELLO/hello/
T{ GC1 -> 58 }T
T{ GC2 -> 48 }T
T{ : GC3 [ GC1 ] LITERAL ; -> }T
T{ GC3 -> 58 }T
T{ : GC4 S" XY" ; -> }T  \ vf: for CBM: s/XY/xy/
T{ GC4 SWAP DROP -> 2 }T
...
: GP1  <# 41 HOLD 42 HOLD 0 0 #> S" BA" S= ;  \ vf: for CBM: s/BA/ba/
T{ GP1 -> <TRUE> }T
...
: GP7
   ...
   MAX-BASE A DO
      I 0 <# #S #>
      1 = SWAP C@ 41 I A - + = AND AND  \ vf: for CBM: s/41/C1/
   LOOP
   R> BASE ! ;
...
\vf No way found yet to inject key strokes into dosbox from Makefile.
\vf T{ ACCEPT-TEST -> }T
