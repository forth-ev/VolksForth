...
Diese Box enthÑlt die Buttons "Cancel" und "Editor", falls der  
   Fehler beim Laden eines Files auftrat. Der Button "Editor"   
   verzweigt in den Editor, "Cancel" zum Kommandointerpreter.   
   "Editor" ist der Defaultwert, der bei DrÅcken von <Return>   
   ausgelîst wird.                                              
Trat der Fehler bei AusfÅhrung von Tastatureingaben auf, gibt   
...
Zugleich wollen wir zeigen, wie einfach unter volksFORTH Alert- 
 Boxen programmiert werden kînnen. Bei unserem Beispiel handelt 
 es sich sogar um einen komplizierten Fall, weil der auszu-     
 gebende String erst in PAD zusammengestellt werden muû.        
                                                                
Ansonsten kînnte eine Alert-Box z.B. so programmiert werden.    
 (Das folgende Beispiel kînnen Sie ausprobieren, indem Sie den  
  Cursor in die nÑchste Zeile setzen und CTRL-L eingeben.       
...
setzt BOXHANDLER als neuen errorhandler. Alle Fehlermeldungen,  
 die Åber abort"  laufen, erscheinen jetzt in Boxen.            
...
ADDSTRING  hÑngt den String bei Adresse string an den String    
 bei $SUM an. Benutzt $ADD aus dem File STRINGS.SCR             
                                                                
BOXHANDLER   gibt den String von ABORT" in einer Alert-Box aus. 
 Maus einschalten und PAD als Ziel fÅr ADDSTRING vorbereiten.   
 Die 3 sorgt fÅr das STOP-Icon in der Box.                      
 Bei HERE steht das Wort, das den Fehler verursacht hat.        
 In die nÑchste Zeile kommt die Fehlermeldung von ABORT"        
