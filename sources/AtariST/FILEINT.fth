...
View-Felder der Worte anschliežend gepatched werden             
(s. STARTUP.SCR)                                                
...
-n ist die Fehlernummer; es wird der zugeh”rige Text ausgedruckt
 ...
 den abort" String gepatched. Dieses Verfahren ist zwar „užerst 
 h„žlich, nichtsdestoweniger aber sehr effektiv.                
                                                                
Prft, ob ein Fehler vorliegt und druckt ggf. den Text aus und  
  ABORTed anschliežend.                                         
...
DTA             ist ein 44 Byte grožer Buffer, in dem einige    
...
SEARCH0         SEARCH0 sucht ein File. C$ ist der Name des File
   ...
   Die DTA enth„lt anschliežend Filenamen, -l„nge usw.          
...
FILEREAD        liest den Block BLOCK an die Adresse BUFF aus   
   dem File FCB. Hinterl„žt eine Fehlernummer.                  
                                                                
FILEWRITE       berschreibt den Block BLOCK mit den Daten ab   
   Adresse BUFF im File FCB. Hinterl„žt eine Fehlernummer.      
...
SETPATH         Setzt PATHES auf den String ab der Adresse ADR, 
   dessen L„nge LEN ist. Anschliežend wird noch ein ; angefgt, 
...
(SEARCHFILE     durchsucht alle in PATHES stehenden Pfade nach  
   ...
   Am Schluž wird auch der leere Pfad (L„nge Null) durchsucht,  
...
(OPEN           ”ffnet das durch FCB angegebene File            
   und speichert LEN dort die Handle und L„nge. Dazu muž es     
...
FILER/W         ist das zentrale Wort fr den Zugriff auf Files.
    ...
    Nummer des Files (0 heižt Direktzugriff) und R/W gibt an, in
...
.DTA            druckt den Inhalt der DTA formattiert aus.      
  ...
  um ein Subdirectory handelt, anschliežend der Name gefolgt    
...
FLUSHFILE       sichert alle zum File FCB geh”renden Blockpuffer
   auf dem Massenspeicher und l”scht sie anschliežend.          
...
FILE            ist ein definierendes Wort, daž einen FCB       
...
FLUSH           schliežt zus„tzlich alle Files..                
...
ADDBLOCK        fgt den Block N am Fileende an.                
  Aužerdem wird ein leerer Buffer mit dieser Nummer angelegt.   
...
REMOVE-FILES    guckt nach, ob ISFILE oder FROMFILE vergessen   
   ...
   umgeschaltet.                                                
   Anschliežend werden alle zu vergessenden Files geschlossen   
...
Mit BLOCKS>FILE l„žt sich eine Folge von Diskettenbl”cken in    
