...
Line_A Variablen. Der Aufruf Åber $A000 liefert unter anderem   
die Basisadresse dieser Variablen zurÅck.                       
                                                                
Wenn diese Definitionen in anderen Programmen mitgenutzt werden 
sollen, mÅssen diese beiden Screens mit                         
...
Bei jeder Line_A Routine lÑût sich am Quelltext sehen, welche   
...
Diese vier Variablen beschreiben das 'Clipping-Window'.  Damit  
 lassen sich alle Ausgaben auf dieses Window beschrÑnken.       
Anzahl der Planes fÅr FÅllmuster                                
Bitmuster fÅr Linien ($FFFF = durchgezogen)                     
Mit diesen vier Variablen werden die Farben der Planes fest-    
 gelegt.                                                        
                                                                
                                                                
Hilfsvariable zur Vereinfachung bei Draw. EnthÑlt die Endkoordi-
 naten der zuletzt gezeichneten Linie.                          
Schreibmodus: 0=over, 1= trans, 2=exor, 3=invtrans              
Bildschirmauflîsung: 0=320x200, 1=320x400, 2=640x400            
                                                                
                                                                
\ *** Block No. 23 Hexblock 17 
\ arrays                                               17sep86we
                                                                
EnthÑlt die Anzahl - 1 der Worte in Arrays fÅr FÅllmuster.      
EnthÑlt die Adresse des aktuellen FÅllmusters.                  
                                                                
Zwei wichtige FÅllmuster: Leer                                  
und voll                                                        
                                                                
Flag, ob die Koordinaten ÅberprÅft werden sollen (Geschwindigk.)
Flag, ob mit Clipping gearbeitet wird.                          
                                                                
speichert die lange Adresse der ZeichsÑtze.                     
...
Wird bei vielen Routinen zu Beginn benutzt.                     
 $A000 Åbergibt in A0 a_base, in A1 a_fonts                     
 Schreibmodus                                                   
 und die Farben der Planes Åbergeben                            
...
zeichnet eine Gerade von (x1,y1) nach (x2,y2).                  
 ...
 Die Werte fÅr x2,y2 werden auch in cur_x und cur_y gemerkt.    
...
zeichnet ein gefÅlltes Rechteck mit x1,y1 als oberer linker Ecke
 und width und height als Breite und Hîhe.                      
 Umrechnung von Breite und Hîhe in Koordinaten                  
                                                                
                                                                
 Adresse des FÅllmusters Åbergeben.                             
                                                                
 Anzahl der Worte im FÅllmuster                                 
 Anzahl der Planes fÅr FÅllmuster                               
...
schaltet Maus-Cursor ein                                        
 CONTRL(1) wird gelîscht und CONTRL(3) auf 1 gesetzt (???)      
 INTIN(0) wird gelîscht, sonst wird die Anzahl der hide-Aufrufe 
 berÅcksichtigt (s.a. c-flag beim entsprechenden VDI-Aufruf)    
                                                                
schaltet Maus-Cursor aus.                                       
                                                                
Damit kann eine eigene Mausform entwickelt werden.              
 Adresse enthÑlt ein Array mit folgendem Aufbau:                
...
Die Copyrasterfunktionen verlangen eine sehr komplexe Parameter-
 Åbergabe. Diese ist im File VDI.SCR an der entsprechenden      
 Stelle enthalten. Da diese Funktion gegenÅber der VDI-Funktion 
 kaum Geschwindigkeitsvorteile bringt, wurde auf die nochmalige 
 Definition hier verzichtet.                                    
                                                                
Wen's interessiert, mîge im File VDI.SCR unter Rasterfunctions  
 nachlesen.                                                     
                                                                
So lÑdt man den entsprechenden Teil der VDI-Bibliothek !        
 Dieser Teil wird schon vom Editor benîtigt und ist daher im    
...
Array mit den Grenzen fÅr die drei Auflîsungsstufen.            
                                                                
flag=0 bei 320x200, flag=1 bei 320x400, flag=2 bei 640x400      
                                                                
                                                                
                                                                
ÅberprÅft, ob x und y innerhalb des Bildschirms liegen.         
 Ansonsten erfolgt Abbruch. Diese PrÅfung kostet Zeit, erspart  
 aber SystemabstÅrze bei falschen Parametern.                   
                                                                
prÅft x und y, wenn checking eingeschaltet ist.                 
...
zeichnet ein ungefÅlltes Rechteck mit der Breite width und Hîhe 
...
sprt_def_blk enthÑlt die Sprite-Daten                           
sprt_sav_blk ist die Adresse des Zwischenspeichers fÅr den Bild-
 schirm. Es werden pro Plane 64 Byte benîtigt.                  
(x,y) ist der 'Hotspot' des Sprites.                            
                                                                
lîscht das Sprite und restauriert den Bildschirm.               
...
zeichnet ein n-Eck mit den Eckpunkten (x1,y1) ... (xn,yn).      
 ...
 Clipping auswerten                                             
 FÅllmuster Åbergeben                                           
                                                                
 FÅllmustermaske                                                
 und Anzahl der Planes Åbergeben                                
 Clipping-Window setzen                                         
                                                                
                                                                
                                                                
 Anzahl der Ecken                                               
 Eckpunkte ins ptsin-Array Åbernehmen                           
 D3 und D4 enthalten die Koordianten des grîûten Punktes        
\ *** Block No. 35 Hexblock 23 
\ polygon forts.                                       17sep86we
                                                                
 fÅr die FÅllfunktion                                           
 Werte Åbergeben und D3,D4 ggf updaten.                         
                                                                
                                                                
                                                                
 ersten Punkt wiederholen, vereinfacht die öbergabe             
                                                                
 $A006 so oft aufrufen, bis das n-Eck vollstÑndig gefÅllt ist.  
