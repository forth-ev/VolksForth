...
In diesem File haben wir Worte untergebracht, die zwar hÑufig   
...
\ Loadscreen fÅr Diverses                              26oct86we
...
\ HÑufig benutzte Definitionen                         26oct86we
...
\ HÑufig benutzte Definitionen II                      26oct86we
...
kompiliert die nÑchsten 2 Screens.                              
                                                                
.STATUS ist ein 'deferred word', das jeweils beim Kompilieren   
 eines Quelltextscreens aufgerufen wird.                        
                                                                
Screen 4 wird nicht mitkompiliert, denn SETVEC muû nach jedem   
 Neustart wieder aufgerufen werden. Falls Sie diese Funktion    
 nutzen wollen, mÅssen Sie nach jedem Laden SETVEC eingeben.    
 (Dazu muû natÅrlich Screen 4 kompiliert worden sein.)          
...
ARGUMENTS   prÅft, ob eine bestimmte (Mindest-)Anzahl von Werten
...
CPUSH       sorgt im Zusammenspiel mit CPULL dafÅr, daû ein     
            String (bzw. ein beliebiger Speicherbereich, z.B.   
            ein Array) nach dem Aufruf einer Funktion wieder    
            die alten Werte erhÑlt. Entspricht dem Wort PUSH,   
            aber fÅr Strings anstelle von Variablen.            
                                                                
                                                                
BELL        Dieses Wort ist selbsterklÑrend !!!                 
BLANK       fÅllt ab addr count Speicherstellen mit Leerzeichen.
...
Vielleicht haben Sie es schon einmal bemerkt. Wenn Sie auf eine 
 ...
 ist, erscheint eine Alert-Box, aber ohne Maus, sodaû Sie den   
 ABBRUCH-Knopf nur durch geduldiges Experimentieren mit der Maus
 erreichen kînnen. Diese Box wird vom Betriebssystem ohne unser 
 Zutun und ohne Einwirkungsmîglichkeit erzeugt.                 
NEWVECTOR Ñndert den zugehîrigen Vector (critical error handler)
 so, daû diese Boxen nicht mehr erscheinen, wohl aber die, in   
 denen z.B. zum Diskettenwechsel aufgefordert wird.             
SETVEC und RESTVEC dienen zum Umschalten zwischen altem und     
 neuen Vector.                                                  
Insbesondere muû BYE den alten Vector wiederherstellen, sonst   
 stÅrzt das System gnadenlos ab.                                
Noch keine besonders elegante Lîsung, aber besser als keine !!  
