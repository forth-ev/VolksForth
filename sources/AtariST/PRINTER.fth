...
Dieses File enthÑlt das Printer-Interface. Die Definitionen fÅr 
die Druckersteuerung mÅssen ggf. an Ihren Drucker angepaût wer- 
den.                                                            
...
Zum Ausdrucken der Quelltexte gibt es die Worte                 
   ...
   listing    ( -- )   wie document, aber fÅr das ganze File    
...
| Create DIN                                                    
Ascii Ñ c,      Ascii î c,      Ascii Å c,      Ascii û c,      
Ascii é c,      Ascii ô c,      Ascii ö c,      Ascii › c,      
...
Eingestellt ist das Druckerinterface auf Epson und kompatible   
 Drucker. Die Steuersequenzen auf den Screens 2, 4 und 5 mÅssen 
 gegebenenfalls auf Ihren Drucker angepaût werden. Bei uns gab  
 es mit verschiedenen Druckern allerdings keine Probleme, da    
 sich inzwischen die meisten Druckerhersteller an die Epson-    
 Steuercodes halten.                                            
                                                                
Arbeiten Sie mit einem IBM-kompatiblen Drucker, muû die Umlaut- 
 wandlung auf Screen 6 wegkommentiert werden.                   
                                                                
ZusÑtzliche 'exotische' Steuersequenzen kînnen nach dem Muster  
...
nur aus stilistischen GrÅnden. Das Folgende liest sich besser.  
                                                                
Hauptausgabewort; gibt ein Zeichen auf den Drucker aus. Es wird 
 gewartet, bis der Drucker bereit ist. (PAUSE fÅr Multitasking) 
                                                                
                                                                
gibt Steuerzeichen an Drucker                                   
                                                                
Steuerzeichen fÅr Drucker. Gegebenenfalls anpassen!             
...
setzt bzw. lîscht Ausgabe komprimierter Schrift.                
                                                                
setzt Zeichenbreite auf 10 bzw. 12 cpi.                         
Zeilenabstand in Zoll.                                          
                                schaltet Super- und Subscript ab
Perforation Åberspringen ein- und ausschalten.                  
Es folgen die Steuercodes fÅr Fettdruck, Kursivschrift, Breit-  
 schrift, Unterstreichen, Subscript und Superscript.            
 Diese mÅssen ggf. an Ihren Drucker angepaût werden.            
 SelbstverstÑndlich kînnen auch weitere FÑhigkeiten Ihres Druk- 
...
Voreinstellung des Druckers auf 'normale' Werte; wird beim      
 Einschalten mit PRINT ausgefÅhrt.                              
...
p! wird neu definiert. Daher brauchen die folgenden Worte p!    
 nicht zu Ñndern, egal, ob mit oder ohne Umlautwandlung gearbei-
 tet wird.                                                      
...
ein Zeichen lîschen (?!)        del                             
neue Seite                      page                            
Drucker auf Zeile und Spalte    at                              
 positionieren; wenn nîtig,                                     
 neue Seite.                                                    
Position feststellen            at?                             
Zeichenkette ausgeben           type                            
                                                                
Damit sind die Worte fÅr eine eigene Output-Struktur vorhanden. 
...
Die folgenden Worte sind von FORTH aus zugÑnglich.              
                                                                
schaltet Ausgabe auf Printer um. (ZurÅckschalten mit DISPLAY)   
...
Druckt die öberschrift der Seite pageno.                        
...
gibt eine Seite aus. Anordnung der Screens auf der Seite:  1 4  
 ...
 LÅcken auf der rechten Seite mit dem Logo-Screen (0)      3 6  
 aufgefÅllt.                                                    
...
PRINTERSEM ist ein Semaphor fÅr das Multitasking, der den Zugang
 auf den Drucker fÅr die einzelnen Tasks regelt.                
                                                                
PTHRU gibt die Screens von  from  bis  to  aus.                 
 AusgabegerÑt merken und Drucker einschalten. Multitasking wird,
...
Vor (oder auch nach) dem Aufruf von spool' muû der Multitasker  
