...
der Buffer auûerhalb des Forth-Systems verwaltet. Die Organi-   
sation ist analog, mit der Ausnahme, daû es kein Updateflag     
gibt, geÑnderte Blîcke also sofort auf die Diskette zurÅckge-   
schrieben werden. Die Benutzung ist vîllig transparent, am      
Anfang muû nur einmal INITRAMDISK aufgerufen werden.            
...
EMPTYRAMDISK  ( -- ) lîscht den Inhalt aller Buffer.            
...
Gibt Offset einer Uservariablen in der Userarea. Dieses         
  Wort gehîrt eigentlich in den Assembler !                     
...
Dieses Wort wird in (CLOSE gepatched. FCB ist die Adresse des   
  zu schlieûenden Files. Alle Blockpuffer dieses Files werden   
  gelîscht.                                                     
Dieses Wort wird in EMPTY-BUFFERS gepatched. Es lîscht alle     
  Ramdiskpuffer                                                 
...
Frage nach der Grîûe der Ramdisk                                
                                                                
\ *** Block No. 16 Hexblock 10 
                                                      bp 17Aug86
                                                                
Zeiger auf den ersten Buffer in der Ramdisk.                    
Beginn des fÅr die Ramdisk allokierten Speicherbereichs         
LÑnge   "   "   "     "        "               "                
                                                                
LÑnge eines Buffers der Ramdisk                                 
...
Diese beiden Worte kînnen noch optimiert werden, da LCMOVE      
 byteweise ÅbertrÑgt, aber auch langwortweise Åbertragen        
 werden kann.                                                   
...
Markiert den Ramdiskbuffer DADDR als Buffer fÅr den Block BLK   
 im File FILE.                                                  
...
Erzeugt einen Buffer fÅr den Blockl BLK des Files FILE in der   
 Ramdisk. Der Inhalt des Buffers steht ab Adresse ADR im System.
 RAMBUF? wird benutzt, um den allokierten Buffer an die erste   
 Stelle zu hÑngen. Der WHILE-Teil wird max. einmal durchlaufen !
\ *** Block No. 21 Hexblock 15 
                                                      bp 17Aug86
                                                                
Lîscht den Buffer LADDR.                                        
                                                                
                                                                
Lîscht alle Ramdiskbuffer, die zum File FCB gehîren.            
...
Erzeugt die Ramdisk. ZunÑchst wird der alte Speicherbereich     
 freigegeben, falls einer allokiert war. Dann wird nach der     
 gewÅnschten Zahl von Buffern gefragt. Es wird ein Speicher=    
...
Die alte R/W-Routine wird natÅrlich auch benîtigt.              
